...
def setLayers(data_source, data_test, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in,
              patchSize, numParts, numStages, deploy=False):
    ...
    if deploy == False:
        nr = caffe.NetSpec()
        nr.data, nr.tops['label_kp'], nr.tops['label_kp_pos'] = \
            L.PoseData(cpmdata_param=dict(backend=1, source=data_source,
                                          batch_size=batch_size), transform_param=transform_param_in,
                       include=dict(phase=caffe.TRAIN), ntop=3)
        train_data = str(nr.to_proto())
        # Test data
        n.data, n.tops['label_kp'], n.tops['label_kp_pos'] = \
            L.PoseData(name='data', cpmdata_param=dict(backend=1, source=data_test, batch_size=batch_size),
                       transform_param=transform_param_in, include=dict(phase=caffe.TEST), ntop=3)
...
def writePrototxts(dataFolder, testFolder, dir, batch_size, num_iter_cnn, layername, kernel, stride, outCH, transform_param_in,
                   base_lr, task_name, proto_folder, weights_folder, label_name, patchSize, numParts, numStages):
    ...
    with open('%s/%s_deploy.prototxt' % (dir,task_name.lower()), 'w') as f:
        ...
        str_to_write = str(setLayers('', '', 0, layername, kernel, stride, outCH, label_name, transform_param_in,
                                     patchSize, numParts, numStages, deploy=True))
        f.write(str_to_write)
