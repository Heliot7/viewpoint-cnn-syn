...
def setLayers(real_data, real_test, syn_data, batch_size, layername, kernel, stride, outCH, label_name, transform_param_real, transform_param_syn,
    ...
    if deploy == False:
        nr = caffe.NetSpec()
        nr.data_real, nr.tops['label_kp'], nr.tops['label_kp_pos'] = \
            L.PoseData(cpmdata_param=dict(backend=1, source=real_data, batch_size=batch_size),
                       transform_param=transform_param_real, include=dict(phase=caffe.TRAIN), ntop=3)
        nr.data_syn, nr.tops['label_vp'] = \
            L.ViewpointData(cpmdata_param=dict(backend=1, source=syn_data, batch_size=batch_size),
                            transform_param=transform_param_syn, include=dict(phase=caffe.TRAIN), ntop=2)
        ...
        n.data, n.tops['label_kp'], n.tops['label_kp_pos'], n.tops['label_vp'] = \
            L.PoseData(name='data', cpmdata_param=dict(backend=1, source=real_test, batch_size=batch_size*2),
                       transform_param=transform_param_real, include=dict(phase=caffe.TEST), ntop=4)
    ...
    for l in range(0, len(layername)):
        if layername[l] == 'C':
            if state == 'image':
                ...
                if conv_counter ==  1 or outCH[l] == numParts: # Fine-tuning for last conv layer of each stage with parts = num obj parts
                    conv_name = 'Mconv%d_stage%d_FT' % (conv_counter, stage)
                    if deploy == False and outCH[l] == numParts and stage == numStages:
                        ...
                        n.tops['real_features%d' % stage] = L.Reshape(n.tops[last_layer],
                                                                      name='propagate_test_real%d' % stage,
                                                                      reshape_param=dict(shape=dict(dim=[0, 0, 0, 0])),
                                                                      include=dict(phase=caffe.TEST))
                        n.tops['syn_features%d' % stage] = L.Reshape(n.tops[last_layer],
                                                                     name='propagate_test_syn%d' % stage,
                                                                     reshape_param=dict(shape=dict(dim=[0, 0, 0, 0])),
                                                                     include=dict(phase=caffe.TEST))
            ...
            if deploy == False:
                if stage < numStages:
                    ...
                    n.tops['real_features%d' % stage] = L.Reshape(n.tops[last_layer], name='propagate_test%d' % stage,
                                                                  ...
                                                                  include=dict(phase=caffe.TEST))
                if stage == 1:
                    n.tops['loss_stage%d' % stage] = L.EuclideanLoss(n.tops['real_features%d' % stage], n.tops[label_name[0]])
                elif stage < numStages:
                    n.tops['loss_stage%d' % stage] = L.EuclideanLoss(n.tops['real_features%d' % stage], n.tops[label_name[1]])
                else: # stage == numStage
...
def writePrototxts(dataFolder, testFolder, dataSynFolder, dir, batch_size, num_iter_cnn, layername, kernel, stride, outCH,
                   transform_param_real, transform_param_syn, base_lr, task_name, proto_folder, weights_folder,
                   label_name, patchSize, numParts, numStages, onlyAZ, binSize):
    ...
    with open('%s/%s_train_test.prototxt' % (dir,task_name.lower()), 'w') as f:
        ...
        str_to_write = setLayers(dataFolder, testFolder, dataSynFolder, batch_size, layername, kernel, stride, outCH,
                                 label_name, transform_param_real, transform_param_syn, patchSize, numParts, numStages,
    ...
    with open('%s/%s_deploy.prototxt' % (dir,task_name.lower()), 'w') as f:
        ...
        str_to_write = str(setLayers('', '', '', 0, layername, kernel, stride, outCH, label_name, transform_param_real,
                                     transform_param_syn, patchSize, numParts, numStages, onlyAZ, binSize, deploy=True))
        f.write(str_to_write)
...
if __name__ == "__main__":
    ...
    writePrototxts(dataFolder, testFolder, dataSynFolder, directory, batch_size, num_iter_cnn, layername, kernel, stride, outCH,
                   transform_param_real, transform_param_syn, base_lr, task_name, directory, weights_folder, label_name,
                   patchSize, numParts, numStages, onlyAZ, binSize)