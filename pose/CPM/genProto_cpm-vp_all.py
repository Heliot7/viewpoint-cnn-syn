...
def setLayers(data_source, data_test, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in,
    ...
    if deploy == False:
        nr = caffe.NetSpec()
        nr.data, nr.tops['label_kp'], nr.tops['label_kp_pos'], nr.tops['label_vp'], nr.tops['label_class'] = \
            L.PoseData(cpmdata_param=dict(backend=1, source=data_source, batch_size=batch_size),
                       transform_param=transform_param_in, include=dict(phase=caffe.TRAIN), ntop=5)
        ...
        n.data, n.tops['label_kp'], n.tops['label_kp_pos'], n.tops['label_vp'], n.tops['label_class'] = \
            L.PoseData(name='data', cpmdata_param=dict(backend=1, source=data_test, batch_size=batch_size),
                       transform_param=transform_param_in, include=dict(phase=caffe.TEST), ntop=5)
    ...
    for l in range(0, len(layername)):
        if layername[l] == 'C':
            ...
            if deploy == False:
                n.tops['mask_loss_stage%d' % stage] = L.MaskOutputs(n.tops[last_layer], n.tops['label_class'],
        ...
        # Add FC layer
            ...
            if deploy == False:
                if layername[l] == 'K':
                    if onlyAZ:
                        n.tops['mask_fc1_stage_vp%d' % numStages] = L.MaskOutputs(n.tops['fc1_stage_vp%d' % numStages], n.tops['label_class'],
    ...
    if deploy == False:
        ...
        return 'name:"CPM-VP_all"\n' + 'layer {' + 'layer {'.join(str(n.to_proto()).split('layer {')[1:])



def writePrototxts(dataFolder, testFolder, dir, batch_size, num_iter_cnn, layername, kernel, stride, outCH,
                   transform_param_in, base_lr, proto_folder, weights_folder, label_name, patchSize, numParts,
                   numClasses, numStages, onlyAZ, binSize):
    ...
    with open('%s/all_train_test.prototxt' % dir, 'w') as f:
        ...
        str_to_write = setLayers(dataFolder, testFolder, batch_size, layername, kernel, stride, outCH, label_name,
                                 transform_param_in, patchSize, numParts, numClasses, numStages, onlyAZ, binSize, deploy=False)
    ...
    with open('%s/all_deploy.prototxt' % dir, 'w') as f:
        ...
        str_to_write = str(setLayers('', '', 0, layername, kernel, stride, outCH, label_name, transform_param_in,
                                     patchSize, numParts, numClasses, numStages, onlyAZ, binSize, deploy=True))
        f.write(str_to_write)
...
if __name__ == "__main__":
    ...
    print "size patch: [" + sys.argv[3] + "," + sys.argv[3] + "]"
    # sys.argv[5]: size batch
    print "size batch: [" + sys.argv[4] + "," + sys.argv[4] + "]"
    # sys.argv[6]: number of iterations CNN
    print "num iter CNN: " + sys.argv[5]
    # sys.argv[7]: number of classes
    print "number of classes: " + sys.argv[6]
    # sys.argv[8]: number of max parts
    print "number of max parts: " + sys.argv[7]
    ...
    num_iter_cnn = int(sys.argv[5])
    numClasses = int(sys.argv[6])
    ...
    writePrototxts(dataFolder, testFolder, directory, batch_size, num_iter_cnn, layername, kernel, stride, outCH,
                   transform_param, base_lr, directory, weights_folder, label_name, patchSize, numParts, numClasses,
                   numStages, onlyAZ, binSize)