...
def setLayers(data_source, data_test, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in,
              patchSize, numParts, numClasses, numStages, deploy=False):
    ...
    if deploy == False:
        nr = caffe.NetSpec()
        nr.data, nr.tops['label_kp'], nr.tops['label_kp_pos'], nr.tops['vp_ignore'], nr.tops['label_class'] = \
        ...
        # Test data
        n.data, n.tops['label_kp'], n.tops['label_kp_pos'], n.tops['vp_ignore'], n.tops['label_class'] = \
        ...
        n.Silence_vp_real = L.Silence(n.tops['vp_ignore'], ntop=0)
...
def writePrototxts(dataFolder, testFolder, dir, batch_size, num_iter_cnn, layername, kernel, stride, outCH, transform_param_in,
                   base_lr, proto_folder, weights_folder, label_name, patchSize, numParts, numClasses, numStages):
    ...
    with open('%s/all_deploy.prototxt' % dir, 'w') as f:
        ...
        str_to_write = str(setLayers('', '', 0, layername, kernel, stride, outCH, label_name, transform_param_in,
                                     patchSize, numParts, numClasses, numStages, deploy=True))
        f.write(str_to_write)
...
if __name__ == "__main__":
    ...
    # sys.argv[10]: keep or not the AR when resizing to crop_size
    ...
    writePrototxts(dataFolder, testFolder,  directory, batch_size, num_iter_cnn, layername, kernel, stride, outCH, transform_param,
                   base_lr, directory, weights_folder, label_name, patchSize, numParts, numClasses, numStages)