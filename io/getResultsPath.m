function [path, name] = getResultsPath(input)
    ...
    extraDA = '';
    if(input.isDA)
        if(strcmpi(input.typeDA,'FMO'))
            extraDA = ['_FMO.' num2str(input.iterResetDA) '.' num2str(input.iterDA)];
...
%         end
    ...
    name = [strZ '[' strClass strDet strPose '] ' strAR strScaleInv srcDataset '(' num2str(input.numSrcTrain ) ')' ...
