function allFeatures = getFeatures(input, dataset_info, data, numAnnotations, extraStrName)
    ...
    for c = 1:length(dataset_info.classes)
        ...
        if(strcmpi(class(input.sourceDataset),'office') || strcmpi(class(input.sourceDataset),'saenko') || ...
            ...
            % Rebuttal ICCV 
            strRebuttal = '_reb';
            if(strcmpi(dataset_info.classes{c}, 'background'))
                strRebuttal = [strRebuttal '_' num2str(input.numImages)];
            end
            strDA = [strPhase '_' strClass(1:3) '_rnd' mat2str(input.seedRand) '_' strSup strRebuttal];
        end
        ...
        if(~strcmpi(class(dataset_info),'Synthetic'))
            matPath = [input.PATH_DATA dataset_info.path 'mat_features\' ...
                dataset_info.classes{c} '_' descriptor '_' strRow '-' strCol strOcc strAR strScale extraStrName strMirror strDA];
            ...
            matPath = [input.PATH_DATA dataset_info.path dataset_info.classes{c} '\' dataset_info.sub_path 'mat_features\' ...
                dataset_info.classes{c} '_' descriptor '_' strRow '-' strCol strAR strScale extraStrName strMirror];
        end
...
%     newFeatures = reshape(allFeatures,[size(allFeatures,1) input.featureInfo.patchSize(2)/32 input.featureInfo.patchSize(2)/32 512]);
%     newFeatures = newFeatures(:,:,:,1:256); % cut to 256 channels only
%     allFeatures = reshape(newFeatures, [size(allFeatures,1) (input.featureInfo.patchSize(2)/32)^2*256]);
end
